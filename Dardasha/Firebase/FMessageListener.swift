...
class FMessageListener {
    ...
    func addMessage(_ message: LocalMessage, memberId: String){
        do {
            try FirestoreReference(.Messages).document(memberId).collection(message.chatRoomId).document(message.id).setData(from: message)
        }catch{
    ...
    func addChannelMessage(_ message: LocalMessage, channel: Channel){
        do {
            try FirestoreReference(.Messages).document(channel.id).collection(channel.id).document(message.id).setData(from: message)
        }catch{
            print("Error saving message to firestore: ",error.localizedDescription)
        }
    }
    ...
    func checkForOldMessages(documentId: String, collectionId: String){
        FirestoreReference(.Messages).document(documentId).collection(collectionId).getDocuments { querySnapshot, error in
    ...
    func listenForNewMessages(_ documentId: String, collectionId: String, lastMessageDate: Date){
        newMessageListener = FirestoreReference(.Messages).document(documentId).collection(collectionId).whereField("date", isGreaterThan: lastMessageDate).addSnapshotListener({ querySnapshot, error in
    ...
    func updateMessageStatus(message: LocalMessage, userId: String){
        ...
        FirestoreReference(.Messages).document(userId).collection(message.chatRoomId).document(message.id).updateData(valus)
    }
    
    func listenForReadStatus(_ documentId: String, collectionId: String, competion: @escaping (_ updatedMessage: LocalMessage) -> Void){
        updatedMessageListener = FirestoreReference(.Messages).document(documentId).collection(collectionId).addSnapshotListener({ querySnapshot, error in
            guard let snapshot = querySnapshot else {return}
